...
class RiderProvider extends Component {
    ...
    componentDidMount() {
        ...
        fetch('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + flickrapikey + `&tags=${this.state.query}&per_page=${this.state.perPage}&page=${}&format=json&nojsoncallback=1`)
            .then(response => response.json())
            .then((data) => {
               console.log(data)
            })
